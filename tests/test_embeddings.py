...
from sourcex.retrieval.embeddings import embed_texts, embed_chunks, embed_query, EmbeddedChunk, EmbeddingError
...
def test_embed_query_success(mock_get_client):
    """T008: Verify successful embedding of a single query."""
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client

    mock_response = MagicMock()
    emb = MagicMock()
    emb.values = [0.5] * config.EMBEDDING_DIMENSIONS
    mock_response.embeddings = [emb]
    mock_client.models.embed_content.return_value = mock_response

    vector = embed_query("What is Xylar?")
    assert len(vector) == config.EMBEDDING_DIMENSIONS

    mock_client.models.embed_content.assert_called_once()
    args, kwargs = mock_client.models.embed_content.call_args
    assert "task: question answering | query: What is Xylar?" in kwargs["contents"]

@patch("sourcex.retrieval.embeddings.get_embedding_client")
def test_embed_query_api_failure(mock_get_client):
    """T008: Verify API failures are properly wrapped."""
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client
    mock_client.models.embed_content.side_effect = APIError("API Rate Limit", 429)

    with pytest.raises(EmbeddingError, match="Gemini API Error:.*"):
        embed_query("Test")

@patch("sourcex.retrieval.embeddings.get_embedding_client")
def test_embed_query_malformed_response(mock_get_client):
    """T008: Verify malformed responses raise an error."""
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client
    mock_client.models.embed_content.return_value = MagicMock(embeddings=[])

    with pytest.raises(EmbeddingError, match="Received empty embedding"):
        embed_query("Test")

@patch("sourcex.retrieval.embeddings.get_embedding_client")
def test_embed_query_invalid_dimensionality(mock_get_client):
    """T008: Verify wrong dimensions raise an error."""
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client
    bad_emb = MagicMock(values=[0.1, 0.2])
    mock_client.models.embed_content.return_value = MagicMock(embeddings=[bad_emb])

    with pytest.raises(EmbeddingError, match="Expected dimensionality"):
        embed_query("Test")

def test_embed_query_empty_input():
    """T008: Verify empty query raises ValueError."""
